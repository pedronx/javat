 ...
 * taking the floor of the result after adding 1/2,
 ...
 */
public class L50_Ex02_SpeedConverter {
    ...
    /**
     * Calculate the values of miles per hour
     */
    public static long toMilesPerHour(double kilometersPerHour) {

        // 1 mile per hour is 1.609 kilometers per hour
        double milePerHour = 1.609;

        if (kilometersPerHour < 0) {
            milePerHour = -1;
        } else {
            milePerHour = Math.round((kilometersPerHour / milePerHour));
        }

        return (long) milePerHour;
    }

    /**
     * Calculates the values of miles per hour and
